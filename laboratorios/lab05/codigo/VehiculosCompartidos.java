...
public class VehiculosCompartidos
{
    ...
    */
    public static GrafoLista leerArchivo(int numeroDePuntos, float p){
          final String nombreDelArchivo = "dataset-ejemplo-U="+numeroDePuntos+"-p="+p+".txt";
          GrafoLista grafo = new GrafoLista(numeroDePuntos);
          try {            
            ...
            while (lineaActual != null){ // Mientras no llegue al fin del archivo. Lee la informacion de las aristas
                ...
                grafo.agregarArco(Integer.parseInt(cadenaParticionada[2]), Integer.parseInt(cadenaParticionada[0]), Integer.parseInt(cadenaParticionada[1]));
                lineaActual = br.readLine();
            }
          }
    ...
    */
    public static LinkedList<LinkedList<Integer>> asignarVehiculos(GrafoLista grafo, float p){
        ...
        for(int i = 2; i < grafo.numVertices(); i++){
            LinkedList<Vertice> caminos = grafo.getLista().get(i);
            LinkedList<Integer> camino = encontrarCamino(i,1,caminos);
            lista.add(camino);
        }
        return lista;
    }

    public static LinkedList<Integer> encontrarCamino(int start, int finish, LinkedList<Vertice> caminos){
    ...
    */
    ...
    public static void main(String[] args){
          ...
          GrafoLista grafo = leerArchivo(numeroDePuntos, p);
